...
const processTheme = (theme: Theme) => {
    ...
    for (const colorName in COLOR_NAMES) {
        ...
        // Main color
        const mainColor = colord(
            theme.colors[color] ?? DEFAULT_THEME.colors[color],
        );
        ...
        processedTheme[`--${color}-content`] = formatHslString(
            colorContent.toHsl(),
        );
        // Focus color
        ...
        processedTheme[`--${color}-focus`] = formatHslString(
            colorFocus.toHsl(),
        );
    }
    ...
    processedTheme['--base-200'] = formatHslString(
        baseColor.darken(0.1).toHsl(),
    );
    processedTheme['--base-300'] = formatHslString(
        baseColor.darken(0.1).darken(0.1).toHsl(),
    );
    ...
    for (const [section, sectionVars] of Object.entries(theme.variables))
        for (const [key, value] of Object.entries(sectionVars))
            processedTheme[`--${section}-${key}`] = value as string;
...
const tailwindFunction: TailwindPluginFn = ({ config, addBase }) => {
    ...
    for (const defaultThemeName of validatedConfig.enabledDefaultThemes) {
        const defaultTheme = DefaultThemes.find(
